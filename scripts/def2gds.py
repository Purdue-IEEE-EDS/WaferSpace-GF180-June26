...
def main():
    ...
    klayout_tech_path = rd_value("klayout_tech")
    tech_name = "gf180mcu"
    if klayout_tech_path and os.path.exists(klayout_tech_path):
        # Check if the technology name is already registered in KLayout
        tech = pya.Technology.technology_by_name(tech_name)
        if tech is None:
            # Only create it if it doesn't exist yet
            tech = pya.Technology.create_technology(tech_name)
            
        tech.load(klayout_tech_path)
        layout.technology_name = tech_name
        print(f"Loaded KLayout technology: {klayout_tech_path}")

    def_map_path = rd_value("def_map")
    if def_map_path and os.path.exists(def_map_path):
        lefdef.map_file = def_map_path
        print(f"Applied layer map profile: {def_map_path}")
    else:
        print("Failed to apply layer map profile")
