...
generate
    ...
    if (CURR_STAGE == 1) begin :stage1
        ...
        logic signed [BITS-1:0] bf_out_re0_pip, bf_out_re1_pip, bf_out_re2_pip, bf_out_re3_pip;
        logic signed [BITS-1:0] bf_out_im0_pip, bf_out_im1_pip, bf_out_im2_pip, bf_out_im3_pip; 

        logic signed [BITS-1:0] rot_out_re, rot_out_im; 
        ...
        logic signed [BITS-1:0] real_tw1, imag_tw1, real_tw2, imag_tw2, real_tw3, imag_tw3;
        logic signed [BITS-1:0] real_tw1_pip, imag_tw1_pip, real_tw2_pip, imag_tw2_pip, real_tw3_pip, imag_tw3_pip;

        logic signed [BITS-1:0] bf_out_re0_pip, bf_out_re1_pip, bf_out_re2_pip, bf_out_re3_pip;
        logic signed [BITS-1:0] bf_out_im0_pip, bf_out_im1_pip, bf_out_im2_pip, bf_out_im3_pip; 
        logic signed [2*BITS-1:0] comm0, comm1, comm2, comm3;

        logic signed [BITS-1:0] rot1_out_re, rot2_out_re, rot3_out_re; 
        logic signed [BITS-1:0] rot1_out_im, rot2_out_im, rot3_out_im; 
        ...
        rotator #(.BITS(16), .DECIMAL(8)) rot1(.clk, .rst, .real_in(bf_out_re1_pip), .imag_in(bf_out_im1_pip), .real_tw(real_tw1), .imag_tw(imag_tw1), .real_out(rot1_out_re), .imag_out(rot1_out_im));
        rotator #(.BITS(16), .DECIMAL(8)) rot2(.clk, .rst, .real_in(bf_out_re2_pip), .imag_in(bf_out_im2_pip), .real_tw(real_tw2), .imag_tw(imag_tw2), .real_out(rot2_out_re), .imag_out(rot2_out_im));
        rotator #(.BITS(16), .DECIMAL(8)) rot3(.clk, .rst, .real_in(bf_out_re3_pip), .imag_in(bf_out_im3_pip), .real_tw(real_tw3), .imag_tw(imag_tw3), .real_out(rot3_out_re), .imag_out(rot3_out_im));
    
        twiddle_rom #(.BITS(16), .DEPTH(1<<(STAGES-CURR_STAGE)), .BRANCH(1)) rom1(.clk, .rst, .in_valid, .tw_re(real_tw1), .tw_im(imag_tw1));
        ...
        logic signed [BITS-1:0] rot_in_re, rot_in_im, rot_out_re, rot_out_im; 
        ...
        always_ff @(posedge clk) begin 
            if (!rst) begin 
                ...
                dout_re0 <= comm0_out_pip[31:16]; 
                dout_re1 <= comm1_out_pip[31:16]; 
                dout_re2 <= comm2_out_pip[31:16]; 
                dout_re3 <= rot_out_re;
                dout_im0 <= comm0_out_pip[15:0]; 
                dout_im1 <= comm1_out_pip[15:0]; 
                dout_im2 <= comm2_out_pip[15:0]; 
                dout_im3 <= rot_out_im; 
            end
        end
        ...
        triv_rot(
            .clk, .rst, 
            .real_in(comm3_out[31:16]), .imag_in(comm3_out[15:0]),
            .real_out(rot_out_re), .imag_out(rot_out_im)
        );
