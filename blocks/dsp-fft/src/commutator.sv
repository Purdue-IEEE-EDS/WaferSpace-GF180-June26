...
(
    ...
    mdc_ram_delay #(.DATA_WIDTH(WIDTH), .DELAY_STAGES(DELAY)) delay1(.clk, .rst, .din(low_in), .dout(low_in_del));
    mdc_ram_delay #(.DATA_WIDTH(WIDTH), .DELAY_STAGES(DELAY)) delay2(.clk, .rst, .din(branch1_sel), .dout(up_out));
